...
onMount(()=>{
    if($default_Wallet.coin_name === "BTC"){
        bet_amount = (0.000028).toFixed(9)
    }
    else if($default_Wallet.coin_name === "ETH"){
        bet_amount = (0.00050).toFixed(9)
    }
    else{
...
$:{
    if($default_Wallet.coin_name === "BTC"){
        if(bet_amount < 0.000028){
            bet_amount = (0.000028).toFixed(8)
        }
        if(bet_amount > 0.14){
            bet_amount = (0.14).toFixed(8)
        }        
    }
    else if($default_Wallet.coin_name === "ETH"){
        if(bet_amount < 0.00050){
            bet_amount = (0.00050).toFixed(8)
        }
        if(bet_amount > 2.53){
            bet_amount = (2.53).toFixed(8)
        }
    }
    else{
        if(bet_amount < 100){
            bet_amount = (100).toFixed(6)
        }
        if(bet_amount > 10000){
            bet_amount = (10000).toFixed(6)
        }
    }
}
...
<div style="margin-top:20px" class="game-control-panel"> 
 {#if $error_msg}
    ...
    <div class="sc-juEPzu lgTgT">
        <div class="sc-ezbkAF gcQjQT input sc-fvxzrP gOLODp sc-gsFzgR fCSgTW game-coininput">
            ...
            <div class="input-control">
                ...
                {#if $handleisLoggin}
                    <img style="border-radius: 50%;" class="coin-icon" alt="" src={$default_Wallet.coin_image}>
                {:else}
