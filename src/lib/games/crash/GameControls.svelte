      ...
      {#if $crashGameType === 1}
        <div class="sc-eqUgKp iuHOYP">
          {#if gameStatus === 1}
            ...
            {:else}
              <button disabled={scriptRunning}
                on:click={handleBetCrash}
                class="sc-iqseJM sc-egiyK cBmlor fnKcEH button button-big sc-ywFzA qPdve">
                <div class="button-inner"><div>Bet</div>
              </div>
            </button>
            {/if}
              ...
              <div class="input-control {isFocused.bet ? 'is-focus' : ''}">
                ...
                <div class="sc-kDTinF bswIvI button-group">
                  <button disabled={betting || inputDisabled} on:click={handleSetAmount("/")}
                    >/2</button>
                  <button disabled={betting || inputDisabled} on:click={handleSetAmount("*")}>x2</button>
                  {#if sliderOpened.classic}
                    ...
                    <div
                      ...
                      class="fix-layer"  style="opacity: 1; transform: none;">
                      <button
                        on:click={handleSetAmount("min")}
                        class={currentAmount <=
                        WalletManager.getInstance().dict[coinName].minAmount ? "active" : ""}>Min</button>
