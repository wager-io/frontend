...
class Currency {
  ...
  addDeduction(e) {
    ...
    if (this.currencyName === "BTC") {
      btc_Wallet.update((wallet) => ({
        ...wallet,
        balance: this.available,
      }));
    } else if (this.currencyName === "WGD") {
      wgdWallet.update((wallet) => ({
        ...wallet,
        balance: this.available,
      }));
    } else if (this.currencyName === "WGF") {
      wgfWallet.update((wallet) => ({
        ...wallet,
        balance: this.available,
      }));
    } else if (this.currencyName === "ETH") {
      ethWallet.update((wallet) => ({
        ...wallet,
        balance: this.available,
      }));
    }
...
export default class WalletManager extends EventEmitter {
  ...
  constructor() {
    ...
    const updateWallet = (wallet) => {
      if (wallet && wallet.coin_name) {
        let minAmount = wallet.coin_name === "WGF" ? 1 : wallet.coin_name === "ETH" ? 0.004 : wallet.coin_name === "BTC" ? 0.0002 : 179878;
          let maxAmount = wallet.coin_name === "WGF" ? 10000 : wallet.coin_name === "ETH" ? 0.1 : wallet.coin_name === "BTC" ? 0.2 : 0.4;
          const usdPrice = wallet.coin_name === "WGF" ? 0 : (wallet.coin_name === "ETH" ? 0.1 : 123);
    ...
    btc_Wallet.subscribe((wallet) => {
      updateWallet(wallet);
    });
    wgfWallet.subscribe((wallet) => {
      updateWallet(wallet);
    });
    wgdWallet.subscribe((wallet) => {
      updateWallet(wallet);
    });
    ethWallet.subscribe((wallet) => {
      updateWallet(wallet);
    });
  ...
  async syncData() {
    ...
    const [wgdwallet, btcWallet, wgfWallet, ethwallet] = await Promise.all([
      handleWGDwallet(),
      handleBTCwallet(),
      handleWGFwallet(),
      handleETHwallet(),
    ...
    const balances = [wgdwallet, btcWallet, wgfWallet, ethwallet].map(
      (wallet) => ({
        amount: wallet.balance,
        currencyName: wallet.coin_name,
        currencyImage: wallet.coin_image,
    ...
    runInAction(() => {
      ...
      balances
        ...
        .forEach((balance) => {
          const minAmount = balance.currencyName === "WGF" ? 100 : balance.currencyName === "ETH" ? 0.0004 : balance.currencyName === "BTC" ? 0.0002 : 199;
          const maxAmount = balance.currencyName === "WGF" ? 10_000 : balance.currencyName === "ETH" ? 0.1  : balance.currencyName === "BTC" ? 0.2 : 30_000;
          const usdPrice = balance.currencyName === "WGF" ? 0 : (balance.currencyName === "ETH" ? 0.1 : 1);
  ...
  amountToFiat(amount, token = this.current.currencyName , fiat = this.preferredFiatCurrency, rates = this.currencyRates) {
    ...
    const usdPrice = token === "WGF" ? 0 : token === "PPL" ? 0.1 : 1; //this.dict[currency].usdPrice
