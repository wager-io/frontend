     ...
     <div class="limit-width">
          <div class="bs9gc4t">
              <div class="swm8knq">
                  ...
                  <div class="ui-select wallet-balance-input-wrap">
                      <button on:click={handleNetworks_isOpen} class="select-trigger">
                          ...
                          <span class="alias">{active_coin.coin_name}</span>
                          <div class="balance">Balance</div>
                          <div class="amount">{(parseFloat(active_coin.balance)).toFixed(8)}</div>
                          <div class="arrow ">
                  ...
                  <div class="select-options-wrap" style="top: 100%; opacity: 1; transform: none;">
                      <div class="ui-scrollview select-options len-9">
                        {#each $coin_list as iu  }
                        {#if iu.coin_name === "BTC" || iu.coin_name === "ETH"}
                            <button class={`select-option ${active_coin.coin_name === iu.coin_name && "active"} `} on:click={()=>handleNetworks(iu)}>{iu.coin_name}</button>
                        {/if}
                        {/each}
                      </div>
                  </div>
            ...
            <div class="withdraw-info">
                ...
                <div class="val">{active_coin.coin_name !== "BTC" ? 0.00001 : 0.00002}  {active_coin.coin_name}</div>
            </div>
            <div class="item">
                ...
                <div class="val cl-primary">
                    <div class="cy2znlo coin notranslate">
                        {#if amount > 6.5}
                         <div class="amount amount-str"> { active_coin.coin_name !== "BTC" ? amount - 1 : amount - 5}</div>
                         {:else}
                         <div class="amount amount-str"> 0</div>
                        {/if}
                        
                    </div> {active_coin.coin_name}</div>
                </div>
            </div>
            ...
            <button disabled={amount && amount < 5.6 || amount > parseFloat(active_coin.balance)} on:click={handleSubmit} class="ui-button button-normal s-conic submit-btn">
...
.select-options:not(.len-1)>.active {
